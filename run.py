...
def main(argv):
    if len(argv) < 1:
        printUsage()
    elif argv[0] == 'test' and argv[1] == 'CodeGenSuite':
        if len(argv) == 2:
            from CodeGenSuite import CodeGenSuite
            getAndTest(argv[1], CodeGenSuite)
        else:
            from CodeGenSuite import CodeGenSuite
            getAndTestFucntion(argv[1] + " - " + argv[2], CodeGenSuite, argv[2])
    else:
        printUsage()
...
def printMiniGo(argv, stream, result):
    print("----------------------------------------------------------------------")
    print(f'{Fore.RED}{argv} - Assignment BTL4 - PPL - HK242 - VO TIEN{Style.RESET_ALL} ')
